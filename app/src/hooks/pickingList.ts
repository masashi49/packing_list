// hooks/pickingList.ts
import { useState, useEffect, useCallback, useRef } from "react";
import { Checklist, ViewType } from "../typs";

const loadFromStorage = (): Checklist[] => {
...
export function usePickingList() {
  const [lists, setLists] = useState<Checklist[]>(loadFromStorage);
  ...
  const saveList = useCallback(() => {
    ...
    resetForm();
    setCurrentView("list");
  }, [resetForm, newListName, newItems, currentView, selectedListIndex, lists]);
